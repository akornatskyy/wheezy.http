...
debian:
	...
	apt-get -y --no-install-recommends install libbz2-dev build-essential \
		...
		libgmp-dev libxml2-dev libxslt1-dev
...
upload:
	...
	sed -i "s/'0.1'/'0.1.$$REV'/" src/wheezy/http/__init__.py ; \
...
test:
	$(PYTEST) -q -x --pep8 --doctest-modules src/wheezy/http
...
test-cover:
	$(PYTEST) -q --cov-report term-missing \
		--cov wheezy.http src/wheezy/http/tests
