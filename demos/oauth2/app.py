...
class OAuth2FacebookClient(OAuth2Client):
    def __init__(
        self, client_id, client_secret, redirect_uri, scope, api_client=None
    ):
...
class OAuth2YahooClient(OAuth2Client):
    def __init__(
        self, client_id, client_secret, redirect_uri, api_client=None
    ):
    ...
    def authorize(self, code):
        client = HTTPClient(
            ...
            headers={
                "Authorization": "Basic "
                + b64encode(self.client_id + ":" + self.client_secret)
            },
        )
        if 200 != client.post(
            "",
...
class WindowsAPIClient(object):
    ...
    def userinfo(self):
        if 200 != self.client.get("me"):
            return None
        m = self.client.json
        return attrdict(email=m.emails.preferred)
...
class LinkedInAPIClient(object):
    ...
    def userinfo(self):
        if 200 != self.client.get("people/~:(email-address)"):
            return None
        m = self.client.json
        return attrdict(email=m.emailAddress)
